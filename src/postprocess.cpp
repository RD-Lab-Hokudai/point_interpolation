...
{
  img = cv::Mat::zeros(grid.rows, grid.cols, CV_64FC1);
  img.forEach<double>([&grid](double &now, const int position[]) -> void {
    double val = grid.at<double>(position[0], position[1]);
    if (val > 1e-9)
    {
      now = 1 - val / 40;
    }
